...
class LauncherService:
    def __init__(self, version_service: VersionsService, github_service: GitHubService, loader_service=None):
        try:
        ...
        try:
            ...
            if not self.auth_service.is_authenticated():
                ...
                if not self.auth_service.authenticate():
                    logging.warning("Microsoft authentication failed - continuing with offline mode")
            ...
            profile_data = list(PROFILE_DATA.values())[0]  # Get the first (and likely only) profile
            ...
            if not version_data:
                logging.warning(f"NeoForge version {neoforge_version} not found, trying vanilla Minecraft...")
                # Extract base MC version from NeoForge version if possible
                base_version = "1.21.1"  # Could parse this from neoforge version in the future
            ...
            if not self._ensure_neoforge_installed_with_loader_service(neoforge_version):
                logging.error("NeoForge installation failed")
                return False
            
            # Build Java command arguments using your profile's Java args
            java_args = self._build_java_command(version_data, neoforge_version, profile_data)
            if not java_args:
            ...
            if return_code is not None:
                ...
                logging.error(f"Minecraft process terminated early with return code: {return_code}")
                return False
            else:
            ...
            if version_manifest_path.exists():
                ...
                if 'inheritsFrom' in version_data:
                    ...
                    if base_version_data:
                        ...
                        if 'arguments' in base_version_data and 'game' in base_version_data['arguments']:
                            version_data['arguments']['game'] = base_version_data['arguments']['game'] + version_data['arguments']['game']
                        
                        # Add base JVM arguments first, then NeoForge-specific ones  
                        if 'arguments' in base_version_data and 'jvm' in base_version_data['arguments']:
                            version_data['arguments']['jvm'] = base_version_data['arguments']['jvm'] + version_data['arguments']['jvm']
                        ...
                        if 'libraries' in base_version_data:
                            ...
                            logging.info(f"Merged libraries: {len(combined_libraries)} total ({len(version_data.get('libraries', []))} NeoForge + {len(base_version_data['libraries'])} base, deduplicated)")
    ...
    def _ensure_neoforge_installed_with_loader_service(self, neoforge_version):
        ...
        try:
            ...
            if neoforge_json.exists() and neoforge_jar.exists():
                logging.info(f"NeoForge {neoforge_version} is already installed - skipping installation")
                return True
            
            if self.loader_service:
                logging.info(f"NeoForge {neoforge_version} not found, using LoaderService to install it")
                self.loader_service.update()
                return True
            else:
                logging.warning("LoaderService not available, falling back to manual installation")
    ...
    def _ensure_neoforge_installed(self, neoforge_version):
        ...
        try:
            ...
            logging.info(f"NeoForge {neoforge_version} not found, attempting installation...")
            ...
            if len(version_parts) >= 2:
                version_number = '-'.join(version_parts[1:])  # Everything after "neoforge-"
    ...
    def _run_neoforge_installer(self, installer_path):
        ...
        try:
            ...
            cmd = ["java", "-jar", str(installer_path), "--install-client", "--minecraft-dir", str(self.minecraft_dir)]
    ...
    def _build_java_command(self, version_data, version, profile_data=None):
        ...
        try:
            ...
            if not java_exe:
                logging.error("No compatible Java version found. Minecraft 1.21.1 with NeoForge requires Java 17 or 21, but Java 24 was found.")
                return None
            ...
            if result.returncode == 0:
                ...
                if version_match:
                    ...
                    if major_version in [17, 21]:
                        ...
                        logging.warning(f"System Java version {major_version} is not compatible. Need Java 17 or 21.")
                        ...
                        try:
                            ...
                            if result.returncode == 0:
                                ...
                                if version_match:
                                    major_version = int(version_match.group(1))
                                    if major_version in [17, 21]:
                                        logging.info(f"Found compatible Java {major_version} at: {java_path}")
                                        return java_path
                        except:
                            continue
                            
            logging.error("No compatible Java version (17 or 21) found. Please install Java 17 or 21.")
            return None
        ...
        try:
                ...
                for lib in version_data['libraries']:
                    if self._should_include_library(lib):
                        ...
                        if lib_path and lib_path.exists():
                            ...
                            logging.warning(f"Library not found: {lib.get('name', 'unknown')} at {lib_path}")
            ...
            if jar_path.exists():
                ...
                if jar_path_str not in seen_libraries:
                    ...
                    logging.warning(f"Main JAR was already in classpath: {jar_path_str}")
            else:
            ...
            logging.info(f"Built classpath with {len(classpath_parts)} unique entries (removed duplicates)")
            
            # Only log classpath details in debug mode to avoid spam
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                if len(classpath) > 2000:  # Only log first part if very long
                    logging.debug(f"Classpath (first 1000 chars): {classpath[:1000]}...")
                else:
            ...
            if 'game' in version_data.get('arguments', {}):
                for arg in version_data['arguments']['game']:
                    if isinstance(arg, str):
                        ...
                        arg = arg.replace('${user_type}', "msa" if auth_info else "offline")  # Use proper user type
                        ...
                        if '${clientid}' in arg or '${auth_xuid}' in arg:
                            logging.debug(f"Skipping argument with clientid/xuid: {arg}")
                            continue
                        args.append(arg)
            else:
                ...
                args.extend([
                    ...
                    "--userType", "msa" if auth_info else "offline"
                ])
